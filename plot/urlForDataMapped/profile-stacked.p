...
set output '../../figures/profiles-urlForDataMapped.pdf'

set title "Solmuhub profile using URL mapper"


plot newhistogram "2-Nodes" lt 1, '../../results/urlMapped-dataClone/2-nodes-1-depth-profile-stacked' u (100.*$2/$9):xtic(1) ti column(2), for [i=3:9] '' using (100.*(column(i)-column(i-1))/$9) ti column(i), \
	 newhistogram "4-Nodes" lt 1, '../../results/urlMapped-dataClone/4-nodes-1-depth-profile-stacked' u (100.*$2/$9):xtic(1) notitle, for [i=3:9] '' using (100.*(column(i)-column(i-1))/$9) notitle, \
	 newhistogram "8-Nodes" lt 1, '../../results/urlMapped-dataClone/8-nodes-1-depth-profile-stacked' u (100.*$2/$9):xtic(1) notitle, for [i=3:9] '' using (100.*(column(i)-column(i-1))/$9) notitle, \
     newhistogram "Local" lt 6, '../../results/urlMapped-dataClone/0-nodes-1-depth-profile-stacked' u (100.*$2/$5):xtic(1) ti column(2), for [i=3:5] '' using (100.*column(i)/$5) title column(i), \
